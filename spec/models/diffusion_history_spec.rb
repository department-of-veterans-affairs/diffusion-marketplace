...
RSpec.describe DiffusionHistory, type: :model do
  ...
  describe 'scopes' do
    let!(:in_progress) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_in_progress) }
    let!(:planning) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_planning) }
    let!(:implementing) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_implementing) }
    let!(:complete) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_complete) }
    let!(:completed) { create_list(:diffusion_history, 3, :with_clinical_resource_hub, :with_status_completed) }
    let!(:implemented) { create_list(:diffusion_history, 3, :with_clinical_resource_hub, :with_status_implemented) }
    let!(:unsuccessful) { create_list(:diffusion_history, 3, :with_clinical_resource_hub, :with_status_unsuccessful) }
    let!(:with_public_practices) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_unsuccessful, practice: create(:practice, :public_practice)) }
    let!(:with_private_practices) { create_list(:diffusion_history, 3, :with_va_facility, :with_status_unsuccessful, practice: create(:practice, :private_practice)) }
