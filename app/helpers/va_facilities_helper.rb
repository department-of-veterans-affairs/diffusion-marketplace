module VaFacilitiesHelper
  ...
  def get_practices_created_count_by_va_facility(va_facility)
    Practice.joins(:practice_origin_facilities).where(published: true, enabled: true, approved: true, practice_origin_facilities: { facility_id: va_facility.station_number }).count
  end
