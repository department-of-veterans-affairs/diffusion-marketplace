...
module PracticesHelper
  ...
  def departments_required_display(practice)
    ...
    when 4
      ...
      "#{practice.number_departments.humanize} department#{practice.number_departments == 1 ? '' : 's'}".humanize
    end
  end

  def duration_of_job_display(practice)
    durations = practice.additional_staffs.map { |as| as.duration_in_weeks&.downcase }
    durations.include?('permanent') ? 'Permanent' : "#{durations.map(&:to_i).sum} weeks" if durations.any?
  end
end
