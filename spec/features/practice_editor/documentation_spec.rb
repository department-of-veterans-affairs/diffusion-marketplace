...
describe 'Practice editor', type: :feature, js: true do
    ...
    describe 'Documentation Page' do
        ...
        it 'should allow the user to add a file upload and a link attachment' do
            find('.additional-document-title-input').set('Test title')
            attach_file('Upload file', @doc_path)
            find('.publication-title-input').set('Test link title')
            find('.publication-link-input').set('www.awesomelink.com')
            @save_button.click
            ...
            expect(page).to have_field('practice[publications_attributes][0][title]', with: 'Test link title')
            expect(page).to have_field('practice[publications_attributes][0][link]', with: 'www.awesomelink.com')
        end
