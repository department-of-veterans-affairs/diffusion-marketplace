...
describe 'Page Builder - Show - Paginated Components - Events', type: :feature, js: true do
  before do
    ...
    @h2 = create(:page_header2_component,subtopic_title: "Visible Past Events")
    ...
    @h3 = create(:page_header2_component,subtopic_title: "Hidden Past Events")
    create(:page_component, page: @page, component: @h3, position: 9)
    @e7 = create(:page_event_component, title: "past event4", start_date: today - 2.days, end_date: today - 1.days, hide_once_passed: true)
    create(:page_component, page: @page, component: @e7, position: 10)
    @e8 = create(:page_event_component, title: "past event5", start_date: today - 2.days, end_date: today - 1.days, hide_once_passed: true)
    create(:page_component, page: @page, component: @e8, position: 11)
    @e9 = create(:page_event_component, title: "past event6", start_date: today - 2.days, end_date: today - 1.days, hide_once_passed: true)
    create(:page_component, page: @page, component: @e9, position: 12)
  ...
  it 'displays events correctly with pagination functionality' do
    ...
    expect(page).to have_content(@h3.subtopic_title)
    expect(page).to have_content("Please check back for more events soon!")
    expect(page).not_to have_content(@e7.title)
    expect(page).not_to have_content(@e8.title)
    expect(page).not_to have_content(@e9.title)
    expect(page).not_to have_css('.dm-load-more-events-2-btn-container')
  end
end
